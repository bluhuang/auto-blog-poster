...
def deploy(config: dict) -> None:
    ...
    2. Clear generated files while preserving ``.git`` and deployment workflows.
    ...
    if not os.path.isdir(deploy_path):
        ...
        subprocess.run(["git", "clone", clone_url, deploy_path], check=True, timeout=120)
    else:
        print(f"Pulling latest for {target_repo} ...")
        subprocess.run(["git", "fetch", "origin"], cwd=deploy_path, check=True, timeout=60)
    ...
    subprocess.run(["git", "config", "user.name", "Auto Blog Poster"], check=False, timeout=10)
    subprocess.run(["git", "config", "user.email", "auto-blog-poster@users.noreply.github.com"], check=False, timeout=10)
    subprocess.run(["git", "add", "."], check=True, timeout=30)

    diff_result = subprocess.run(["git", "diff", "--cached", "--quiet"], timeout=10)
    if diff_result.returncode == 0:
        ...
        subprocess.run(["git", "push", "origin", target_branch], check=True, timeout=120)
...
def _ensure_branch(repo_path: str, branch: str) -> None:
    ...
    if branch in branches or f"remotes/origin/{branch}" in branches:
        subprocess.run(["git", "checkout", branch], cwd=repo_path, check=True, timeout=30)
        subprocess.run(["git", "pull", "origin", branch], cwd=repo_path, check=True, timeout=60)
        print(f"Checked out existing branch: {branch}")
    else:
        print(f"Creating orphan branch: {branch}")
        subprocess.run(["git", "checkout", "--orphan", branch], cwd=repo_path, check=True, timeout=30)
        subprocess.run(["git", "rm", "-rf", "."], cwd=repo_path, check=False, timeout=30)
        subprocess.run(["git", "commit", "--allow-empty", "-m", f"Init {branch}"], cwd=repo_path, check=True, timeout=30)
        subprocess.run(["git", "push", "origin", branch], cwd=repo_path, check=True, timeout=120)
        print(f"Created and pushed orphan branch: {branch}")


def _clear_repo(repo_path: str) -> None:
    """Remove generated files while preserving Git metadata and workflows."""
    preserved_entries = {".git", ".github"}
    for entry in os.listdir(repo_path):
        if entry in preserved_entries:
            continue
        ...
        if os.path.isdir(full):
            ...
            os.remove(full)
    print("Cleared deploy repo (kept .git and .github workflows).")


def _copytree(src: str, dst: str) -> None:
    """Recursively copy all content from *src* into *dst*."""
